...
const ChatBox = () => {
  ...
  // END SWIPE (trigger delete)
  const handleTouchEnd = (e, message) => {
    const touchEndX = e.changedTouches[0].clientX;
    const deltaX = touchStartX.current - touchEndX;

    const swipedLeft = deltaX > 70;
    const swipedRight = deltaX < -70;

    if (swipedLeft && message.from_user_id === myId) {
      handleDeleteMessage(message._id);
    }
    if (swipedRight && message.from_user_id === myId) {
      setEditModal({
        open: true,
  ...
  return (
    user && (
      <div className="flex flex-col h-screen">
        ...
        {/* Edit Modal */}
        {editModal.open && (
          <div className="fixed inset-0 bg-black/30 backdrop-blur-md flex justify-center items-center z-[9999] animate-fadeIn">
            <div className="bg-white/80 backdrop-blur-xl rounded-2xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] w-11/12 max-w-md p-6 animate-scaleIn">
